describe('Visit site', () => {
  ...
  it('Should display page upon load (title, form fields, button, & 1 trick card)', () => {
  ...
  it('Should take inputs from user and show updated inputs in each field', () => {
  ...
  it('Should generate a new trick card when user fills out form', () => {
    cy.get('#trickStance').select('regular')
    cy.get('#name').type('cool doggo')    
    cy.get('#trickObstacle').select('flatground')
    cy.get('#tutorial').type('youtube.com')
